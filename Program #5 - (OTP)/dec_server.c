 ...
 *         		 and use that to decrypt messages through the server side.
 *********************************************************************************/
 ...
 * 			  aspect by decypting message send by the client
	...
	while(1) {
		...
		if (process_run <= 5) {												// If the number of process running is less than 5
			...
			if (pid == -1) {													// Exit if there is an error when forking
				...
				char text[80000], key[80000], keyword[80000], decipher[80000];		
				...
				if (strcmp(keyword, "dec_client") != 0) {				// If keyword doesn't match, send it back to client
					send(connect_sock, "\n", 2, 0);						// Send that connecting to wrong server
					exit(1);
				}

				// Time to decipher the text
				memset(decipher, '\0', sizeof(decipher));				// Clear out the decipher string
				for (int i = 0; i < strlen(text); i++) {				// Loop through the plaintext
					...
					sum = (n1 - n2);											// Subtract those two number together
					if (sum < 0) {												// If the sum is 0
						sum += 27;												// Add the result with 27
					}
					decipher[i] = alphabet[sum];							// Assign the cipher code char at that index from the alphabet
				}

				decipher[strlen(decipher)] = '\n';						// Assign a newline at the end of the decipher text

				// Send data back to dec_client
				do	{																// Do a do-while loop for sending data
					int tmp_WR = 0;															// Create a tmp_WR
					tmp_WR = send(connect_sock, decipher, strlen(decipher), 0);	// Send data and assign an int
					...
					if (char_WR < strlen(decipher)) {								// If the data have not finish, give warning
					...
					else if (char_WR >= strlen(decipher)) {						// If all data has send, break out of the loop
						data_ck = true;
					}
				} while (data_ck == false);
